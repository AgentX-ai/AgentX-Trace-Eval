...
export type SemanticJudge = (rubric: string, text: string) => Promise<boolean>;
...
async function evaluateDetector(condition: PatternCondition, text: string, semanticJudge: SemanticJudge): Promise<boolean> {
  ...
  return condition.caseSensitive ? text.includes(value) : text.toLowerCase().includes(value.toLowerCase());
}
...
export async function evaluatePatternConditions({
  ...
  for (const condition of conditions) {
    ...
    let value = await evaluateDetector(condition, text, semanticJudge);
    if (condition.negate) {
      value = !value;
    }
    if (acc === null) {
